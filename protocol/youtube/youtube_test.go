...
func TestWebm(t *testing.T) {
	r, err := os.Open("wake.webm")
	if err != nil {
		t.Fatal(err)
	}
	var m webm.WebM
	wr, err := webm.Parse(r, &m)
	if err != nil {
		t.Fatal(err)
	}
	_ = wr
	...
	/*
		...
		if err != nil && err.Error() == "Reached payload" {
			...
			wr := newReader(payload, m.Segment.Cues.CuePoint, sh.Offset)
			err = nil
		}
		if err != nil {
			t.Fatal(err)
		}
		fmt.Println(m.Header.DocType)
	//*/
}
